...
/*
 函数
 */
//函数是完成一段独立任务功能代码的片段 你可以使用命名来标识某个函数的功能 这个名字可以用来在需要使用这个“函数功能”的时候调用 swift的函数非常的灵活 可以表示任何的函数 包含没有参数的C函数风格的函数 包括复杂的带局部(函数内部使用)和外部(返回给外部)参数名的objective-c的风格参数 参数可以提供默认值来简化函数的调用 参数还可以做传入的参数 也可以当做传出的参数使用
//swift中 函数也是存在类型的:函数的类型是由参数类型和返回值类型组成的类型 你可以将函数的类型当做其他变量类型一样处理、也就可以将函数当做别的函数的参数来使用、同时也可以将函数当做另外一个函数的返回值来处理、函数的定义同样可以写在其他的函数定义内部作为嵌套函数 这样可以达到功能封装的要求
//1 函数的定义和调用:当你定义一个函数的时候 你可以定义一个或者多个有名字和类型的值作为函数的参数 也可以定义函数类型作为函数的返回值类型 函数名是用来描述某个函数功能的方法名 调用函数时候需要传入与函数匹配的的输入值(实参)
//返回值不需要定义名称 但需要定义返回值的类型 (_)表示忽略某个参数
...
//以上所有的信息的汇总就成为函数的定义 并且以func作为函数的前缀 返回值:使用->后面跟着返回值类型的名称方式来表示返回值
...
//2.2 多参数的函数 在括号内使用逗号分割来区别传入的参数值和类型 当函数定义的参数个数的不同的时候 方法名可以相同
...
greet2(person: "caoya")
//函数的返回值可以被忽略不用 返回值可以被忽略 但是定义了返回值的函数必须返回一个值 如果没有返回值导致编译错误
...
//3 函数参数标签和参数名称:每一个函数参数都有一个参数标签(参数标签在每个参数名称的最前面定义)和参数名称 参数标签在调用函数的时候使用 调用的时候需要将函数标签写在对应的参数前面 参数名称在函数的实现中使用 默认情况下参数名称和参数标签用同一个名称表达func some(someFunction:Int) 中参数标签和参数名称都是someFunction
...
//3.5 可变参数: 一个可变参数可以接受零个或者多个值 函数调用的时候你可以指定函数参数可以传入不确定个数的值(传入的时候使用逗号隔开) 通过在常量参数类型后面添加(...)的方式来定义可变参数 可变参数的传入值在函数体中变为此类型的一个数组 一个函数最多只能存在一个可变的参数
...
//定义输入输出参数 在参数的类型前面添加inout关键字 (传入传出参数的作用:)一个输入输出参数有传入函数的值，这个值被函数修改，然后被传出函数，替换原来的值 原来值会被修改 不能使常量值和字面量(因为它们是不可变的) 并且调用函数时候需要在参数前面添加&符号表示这个值是可以被函数修改的 输入输出参数是函数对函数体外产生影响的另一种方式 和返回值不一样
//注意:输入输出参数不能存在默认值、而且不能在变量前面添加 inout关键字
...
//4.1 使用函数类型:使用函数类型和其他的类型使用一样、你可以定义一个类型为函数类型的常量或者变量 并将函数赋值给定义的常量或者变量
