...
//6 字符串是值类型: Swift 的String类型是值类型 创建新的字符串时 当进行赋值操作或者函数方法传递 会进行值的拷贝 任何情况下会对已有的字符串进行创建新的副本  swift 默认字符串拷贝的方式保证了在函数/方法中传递的是字符串的值。 很明显无论该值来自于哪里，都是您独自拥有的。 您可以确信传递的字符串不会被修改，除非你自己去修改它
...
//10 “每一个字符串都有一个关联的索引index类型 这个index对应着每个character在字符串的位置”   使用startIndex属性可以获取一个String的第一个Character的索引。使用endIndex属性可以获取最后一个Character的"后一个位置的索引"。因此，endIndex属性不能作为一个字符串的有效下标。如果String是空串，startIndex和endIndex是相等的  调用 String 的 index(before:) 或 index(after:) 方法，可以立即得到前面或后面的一个索引。您还可以通过调用 index(_:offsetBy:) 方法来获取对应偏移量的索引，这种方式可以避免多次调用 index(before:) 或 index(after:) 方法
...
//11 indices: 会创建一个包含全部索引的范围(Range)，用来在一个字符串中访问单个字符
for index in greeting.indices {
   
    print("greeting character \(greeting[index])")
}

//注意:上述的方法同意适合使用在Array Dictionary  Set 中
