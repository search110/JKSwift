...
/*
 ...
 注意:"转换没有真的改变实例或它的值"。原本的实例保持不变；只是简单地把当时转换的类型作为被转换成的类型来使用
...
//Any类型可以表示所有类型的值，包括可选类型。当你希望用Any类型来表示一个可选值的时候、swift会给你一个警告、如果你确实希望使用Any类型来表示可选类型、你可以使用as操作符来"显示"的转换为Any类型
